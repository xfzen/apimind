...
function AccessRoute() { const { instanceId = '' } = useParams(); const state = useAsync(() => getInstance(instanceId), [instanceId]); if (state.loading) return <Spin />; if (!state.data) return <Result status="error" title="无法加载实例" />; if (!state.data.manifest) return <Result status="info" title="尚未声明产品 Manifest" subTitle="产品接受 Manifest 后才可配置固定角色。" />; return <RoleBindingEditor instanceId={instanceId} roles={state.data.manifest.roles} /> }

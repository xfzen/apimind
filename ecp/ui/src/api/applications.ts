...
type ManifestBody = { capabilities?: string[]; resources?: Array<{ type: string; actions: string[] }>; roles?: string[] }
function parseManifest(value: ManifestWire): Manifest { const body = JSON.parse(value.body) as ManifestBody; return { resource_types: (body.resources ?? []).map((item) => item.type), roles: body.roles ?? [], actions: [...new Set((body.resources ?? []).flatMap((item) => item.actions))], capabilities: body.capabilities ?? [], version: value.version } }
...
export async function getInstance(id: string) { const value = (await apiClient.get<InstanceWire>(`/instances/${encodeURIComponent(id)}`)).data; const manifest = parseManifest((await apiClient.get<ManifestWire>(`/applications/${encodeURIComponent(value.application_id)}/manifest`)).data); return { id: value.id, application_id: value.application_id, name: value.instance_key, base_url: value.canonical_url, state: value.status, manifest } }

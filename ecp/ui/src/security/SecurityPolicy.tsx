...
export function SecurityPolicy({ instanceId = '', load }: { instanceId?: string; load?: () => Promise<SecurityPolicySchema> }) {
  const state = useAsync(() => load ? load() : getSecurityPolicy(instanceId), [instanceId, load])
  if (state.loading) return <Spin />
  if (!state.data) return <Result status="error" title="无法加载安全策略" />

...
class ProjectData extends Component {
  ...
  // url导入上传
  onUrlUpload = async () => {
    if (!this.state.curImportType || !this.importDataModule[this.state.curImportType]) {
      return message.error('请选择导入数据的方式');
    }
  ...
  render() {
    ...
    let exportUrl =
      ...
      this.exportDataModule[this.state.curExportType] &&
      this.exportDataModule[this.state.curExportType].route;
          ...
          <div className="postman-dataImport">
            <div className="dataImportCon">
              ...
              <div className="dataImportTile">
                ...
                >
                  {Object.keys(this.importDataModule).map(name => {
                    return (
                      <Option key={name} value={name}>
                        {this.importDataModule[name].name}
                      </Option>
                    );
                  })}
                </Select>
                    ...
                    <Dragger {...uploadMess}>
                      ...
                      <p
                        ...
                        dangerouslySetInnerHTML={{
                          __html: sanitizeHTML(
                            this.state.curImportType && this.importDataModule[this.state.curImportType]
                              ? this.importDataModule[this.state.curImportType].desc
                              : ''
                          )
                        }}
                      />
            ...
            <div
              ...
              style={{
                marginLeft: '20px',
                display: Object.keys(this.exportDataModule).length > 0 ? '' : 'none'
              }}
            >
              <div>
                <h3>数据导出</h3>
              ...
              <div className="dataImportTile">
                ...
                >
                  {Object.keys(this.exportDataModule).map(name => {
                    return (
                      <Option key={name} value={name}>
                        {this.exportDataModule[name].name}
                      </Option>
                    );
                  })}
                </Select>
              ...
              <div className="export-content">
                {this.state.curExportType ? (
                  <div>
                    <p className="export-desc">{this.exportDataModule[this.state.curExportType].desc}</p>
                    <a 
                      target="_blank"
                      rel="noopener noreferrer"
                      href={exportHref}>

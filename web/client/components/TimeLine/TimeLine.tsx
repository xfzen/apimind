...
import showDiffMsg from '../../../common/diff-view';
import type { CurrentDiffData } from '../../../common/diff-view';
...
import type { NewsData } from '../../reducer/modules/news';
import type { ApiResponse } from '../../types/api';
import { asLegacyClassDecorator } from '../../types/legacyDecorators';

interface LegacyOptionProps {
  children?: ReactNode;
  value: string;
  title?: string;
  path?: string;
...
interface LegacyAutoCompleteProps {
  ...
  filterOption?: (inputValue: string, option?: { props: LegacyOptionProps }) => boolean;
}

const LegacyAutoComplete = AutoComplete as unknown as ComponentType<LegacyAutoCompleteProps> & {
  Option: ComponentType<LegacyOptionProps>;
  OptGroup: ComponentType<{ children?: ReactNode; label: string }>;
};
const { Option, OptGroup } = LegacyAutoComplete;

interface AddDiffViewProps {
  title: string;
  content: string | null | undefined;
  className?: string;
}

const AddDiffView = (props: AddDiffViewProps) => {
...
interface TimelineNewsItem {
  uid: string | number;
  add_time: number;
  type: keyof typeof logTypeLabels;
  content: string;
...
interface TimeTreeProps {
  newsData: NewsData;
  fetchNewsData: (typeid: string | number, type: string, page: number, limit?: number, selectValue?: unknown) => Promise<unknown>;
  fetchMoreNews: (typeid: string | number, type: string, page: number, limit?: number, selectValue?: unknown) => Promise<unknown>;
  loading?: boolean;
  curpage: number;
  typeid: string | number;
  curUid: number;
  type: string;
  fetchInterfaceList: (params: Record<string, unknown>) => Promise<{
    payload: { data: ApiResponse<{ list: ApiListItem[]; count: number }> };
  }>;
}
...
interface TimeTreeState {
  ...
  loading: boolean;
  visible: boolean;
  curDiffData: CurrentDiffData | null;
  apiList: ApiListItem[];
}
...
const connectTimeTree = asLegacyClassDecorator(connect(
  (state: RootState) => ({
    newsData: state.news.newsData,
    curpage: state.news.curpage,
    curUid: state.user.uid
  }),
  { fetchNewsData, fetchMoreNews, fetchInterfaceList }
));

@connectTimeTree
class TimeTree extends Component<TimeTreeProps, TimeTreeState> {
  static propTypes = {
  ...
  constructor(props: TimeTreeProps) {
    super(props);
    this.state = {
      bidden: '',
      loading: false,
  ...
  render() {
    const newsItems = this.props.newsData
      ? (this.props.newsData.list as unknown as TimelineNewsItem[])
      : [];
    let data: ReactNode;

    const curDiffData = this.state.curDiffData;
    const children = this.state.apiList.map(item => {
      const method = (item.method ? item.method.toLowerCase() : 'get') as keyof typeof variable.METHOD_COLOR;
      const methodColor = variable.METHOD_COLOR[method];
      return (
    ...
    if (newsItems.length) {
      data = newsItems.map((item: TimelineNewsItem, i: number) => {
          ...
          <Timeline.Item
            dot={(() => (
              <Link to={`/user/profile/${item.uid}`}>
                <Avatar src={buildApiUrl(`/api/user/avatar?uid=${item.uid}`)} />
              </Link>
            ))()}
            key={i}
          >
            <div className="logMesHeade">
              ...
              <span className="logtype">{logTypeLabels[item.type]}动态</span>
              <span className="logtime">{formatTime(item.add_time)}</span>
            </div>
            <span className="logcontent" dangerouslySetInnerHTML={{ __html: sanitizeHTML(item.content) }} />
            <div style={{ padding: '10px 0 0 10px' }}>
              {interfaceDiff && item.data && <Button onClick={() => this.openDiff(item.data!)}>改动详情</Button>}
            </div>
          </Timeline.Item>
        );
      });
            ...
            <Col span="10">
              <LegacyAutoComplete
                ...
                filterOption={(inputValue, options) => {
                  if (!options || options.props.value == '') return true;
                  if (

...
class ProjectInterfaceSync extends Component {
  ...
  validSwaggerUrl = async (rule, value) => {
    if(!value)return Promise.resolve();
    try{
      await this.props.handleSwaggerUrlData(value);
    } catch(e) {
      return Promise.reject(new Error('swagger地址不正确'));
    } 
  ...
  sync_cronCheck(rule, value){
    if(!value)return Promise.resolve();
    value = value.trim();
    if(value.split(/ +/).length > 5){
      return Promise.reject(new Error('不支持秒级别的设置，建议使用 "*/10 * * * *" ,每隔10分钟更新'))
    }
    return Promise.resolve();
  }
      ...
      <div className="m-panel">
        <Form form={this.props.form}>
          ...
          <div>
            ...
            <FormItem
              {...formItemLayout}
              label={<span>类cron风格表达式(默认10分钟更新一次)&nbsp;<a href="https://blog.csdn.net/shouldnotappearcalm/article/details/89469047">参考</a></span>}
              name="sync_cron"
              rules={[
                  {
                    required: true,
          ...
          <FormItem {...tailFormItemLayout}>
            <Button type="primary" htmlType="submit" icon={<Icon type="save" />} size="large" onClick={this.handleSubmit}>
              保存
            </Button>
          </FormItem>
        </Form>
...
function ProjectInterfaceSyncForm(props) {
  const [form] = Form.useForm();
  return <ProjectInterfaceSync {...props} form={form} />;
}

export default ProjectInterfaceSyncForm;
